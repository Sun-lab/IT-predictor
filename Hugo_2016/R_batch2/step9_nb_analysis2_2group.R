...
yhat_med = beta0+beta1_med*xnew

resid1 = abs(y[which(y > ymin & y < ymax)] - yhat)
resid1_med = abs(y[which(y > ymin & y < ymax)] - yhat_med)

df1 = data.frame(resid=resid1, resposne=clinic$Response2[which(y > ymin & y < ymax)])
df1_med = data.frame(resid=resid1_med, resposne=clinic$Response2[which(y > ymin & y < ymax)])

g3_lin = ggplot(df1, aes(x=resposne, y=resid)) + 
      geom_boxplot() + 
      geom_jitter(position=position_jitter(0.2), size=1) + 
      labs(title="", x="Response", y = "|Residuals| linear") 


g3_med = ggplot(df1_med, aes(x=resposne, y=resid)) + 
  geom_boxplot() + 
  geom_jitter(position=position_jitter(0.2), size=1) + 
  labs(title="", x="Response", y = "|Residuals| medquant") 

l = mget(c("g0", "g1", "g2_lin", "g2_med"))

ggsave(paste("figures/scatter_noPR_riazLine_nb_vs_response_ymax", toString(ymax), ".pdf", sep =""),
       width=4.5, height=9, 
       marrangeGrob(grobs = l, nrow=3, ncol=1))

ggsave(paste("figures/boxplot_noPR_riazLine_nb_res_vs_response_linear_ymax_", toString(ymax), ".pdf", sep =""), 
       g3_lin, width=3, height=3)

ggsave(paste("figures/boxplot_noPR_riazLine_nb_res_vs_response_medquant_ymax", toString(ymax), ".pdf", sep =""), 
       g3_med, width=3, height=3)

#--------------------------------------------------------------------
# save updated clinical information
#--------------------------------------------------------------------

write.table(clinic, file = "output/clinic_info_noPR.txt", append = FALSE, 
