...
First try to fit two models, one is the standard MAST model and the other one is a mixed effect model that accounts for the dependency of cells that are from the same patient. For this dataset with around 10,000 genes and 14,743 cells, MAST takes around 5-7G memory per thread. With 5 threads, standard MAST takes about 5 minutes, and subsequent lrt takes about 5 minutes. zlm with mixed effect takes about 1 hour, and subsequent lrt takes about 30 minutes. 
