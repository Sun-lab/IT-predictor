...
plot(unlist(count.df[60488,]), unlist(count.df[60485,]), 
     xlab="alignment_not_unique", ylab="ambiguous")

plot(unlist(count.df[60488,]), unlist(count.df[60484,]), 
     xlab="alignment_not_unique", ylab="no_feature")

plot(colSums(count.df[1:60483,]), unlist(count.df[60488,]),  
     xlab="total read-depth", ylab="alignment_not_unique")
...
saveRDS(count.df, file="raw_counts.rds")
```
...
```{r}
# Some ensembl IDs are missing hgnc symbols
...
Filter data and singature matrix using the genes shared between TCGA data and the signature matrix `SF2018_sig_gene_matrix.txt`: 514 genes in common.
...
Filter data and singature matrix using the genes shared between TCGA data and the signature matrix `SF2018_sig_gene_matrix_filtered_cells.txt`: 514 genes in common.

```{r}
fname = "../SF2018_analysis/output/SF2018_sig_gene_matrix_filtered_cells.txt"
SF2018_few0_sigmat = read.table(fname, header = TRUE, sep = "\t", as.is = TRUE)
dim(SF2018_few0_sigmat)
SF2018_few0_sigmat[1:2,1:5]

genes2use    = intersect(SF2018_few0_sigmat$geneSymbol, SKCM_TPM_filtered$geneSymbol)
match.TCGA   = match(genes2use, SKCM_TPM_filtered$geneSymbol)
match.SF2018_filtered = match(genes2use, SF2018_few0_sigmat$geneSymbol)

SKCM_TPM_SF2018_few0 = SKCM_TPM_filtered[match.TCGA,]
dim(SKCM_TPM_SF2018_few0)
SKCM_TPM_SF2018_few0[1:2,1:5]

SKCM_SF2018_few0_sigmat = SF2018_few0_sigmat[match.SF2018_filtered,]
dim(SKCM_SF2018_few0_sigmat)
SKCM_SF2018_few0_sigmat[1:2,1:5]
```
...
sig_file_SF2018_filtered = file.path(CBSTfolder, 
                                     "signature_gene_SF2018_filtered_cells.txt")
mix_file_SF2018_filtered = file.path(CBSTfolder, 
                                     "observed_TPM_SF2018_filtered_cells.txt")
...
write.table(SKCM_TPM_SF2018_few0, file = mix_file_SF2018_filtered,
            quote = FALSE, row.names = FALSE, sep="\t")

write.table(SKCM_SF2018_few0_sigmat, file = sig_file_SF2018_filtered,
            quote = FALSE, row.names = FALSE, sep="\t")
```

# Run ICeDT

Use SF2018 signature matirx
```{r}
...
Use SF2018 signature matirx after selecting 200 cells with largest proportion of non-zero expression for each cell type.

```{r}
SKCM_TPM_SF2018_mtrx = data.matrix(SKCM_TPM_SF2018_few0[,-1])
rownames(SKCM_TPM_SF2018_mtrx) = SKCM_TPM_SF2018_few0$geneSymbol

SKCM_SF2018_sigmat_mtrx = data.matrix(SKCM_SF2018_few0_sigmat[,-1])
rownames(SKCM_SF2018_sigmat_mtrx) = SKCM_SF2018_few0_sigmat$geneSymbol
...
date()
icedt_SF2018_filtered = ICeDT(Y = SKCM_TPM_SF2018_mtrx, 
                              Z = SKCM_SF2018_sigmat_mtrx, 
                              tumorPurity = NULL, refVar = NULL)
date()

summary(icedt_SF2018_filtered$cProp)

saveRDS(icedt_SF2018_filtered, "ICeDT/SKCM_ICeDT_SF2018_filtered_cells.rds")
```

Use LM22 signature matrix
